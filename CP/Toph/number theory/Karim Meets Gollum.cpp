/*
solution 1: 
        factorization with pollard rho algorithm
...
/*
problem 2: Normal factorization with bitwise sieve
*/
#include<bits/stdc++.h>
using namespace std;
#define ll long long
...
ll bigmod(ll a, ll p) {
    ...
    if(p == 0) return 1LL;
        ...
        ll ret = bigmod(a, p-1);
        return (a * ret%mod) % mod;
    }
}

void f(ll pos, ll mult) {
    if(pos == factor.size()) {
        ans = (ans + bigmod(mult, k)) % mod;
        return ;
    }
    f(pos+1, mult);
    ll x = factor[pos].ff, y = factor[pos].ss;
    for(ll i = 1;i <= y; i++) {
        mult *= x;
        f(pos+1, mult);
    }
